...
namespace Physics {
	...
	*/
	...
	Physics::PhysicsVector<3> scaledByVector(const Physics::PhysicsVector<3>& toScale, const Physics::PhysicsVector<3>& scaleBy) {
		Physics::PhysicsVector<3> output;
		for (int i = 0; i < 3; ++i) {
	...
	Physics::PhysicsVector<3> randVector(double inMin, double inMax) {
		...
		return Physics::PhysicsVector<3>{ distribution(mersenne), distribution(mersenne), distribution(mersenne) };
	}
	...
	Physics::PhysicsVector<3> randInUnitSphere() {
		...
		return Physics::PhysicsVector<3>{ std::sin(phi)* std::cos(theta), std::sin(phi)* std::sin(theta), std::cos(phi) };
	...
	Physics::PhysicsVector<3> randInUnitDisk() {
		while (true) {
			Physics::PhysicsVector<3> Vec{ randVector(-1,1) };	//Start off with a vector on the unit cube
	...
	Physics::PhysicsVector<3> refract(const Physics::PhysicsVector<3>& inR, const Physics::PhysicsVector<3>& inNormal, const double etaOverEtaPrime) {
		Physics::PhysicsVector<3> unitVectorR{ inR.getUnitVector() };
		...
		Physics::PhysicsVector<3> rPrimePerp{ (unitVectorR + inNormal.scaledBy(cosTheta)).scaledBy(etaOverEtaPrime) };
		Physics::PhysicsVector<3> rPrimeParallel{ inNormal.scaledBy(-sqrt(fabs(1.0 - rPrimePerp.lengthSquared()))) };
